...
func (b *azureAuthBackend) verifyResource(ctx context.Context, subscriptionID, resourceGroupName, vmName, vmssName, resourceID string, claims *additionalClaims, role *azureRole) error {
	...
	case vmssName != "":
		client, err := b.provider.VMSSClient(ctx, b.Logger(), b.System(), subscriptionID)
		if err != nil {
			return err
		}
		...
		for userIdentityID, userIdentity := range vmss.Identity.UserAssignedIdentities {
			...
			msiClient, err := b.provider.MSIClient(ctx, b.Logger(), b.System(), msiID.SubscriptionID)
			if err != nil {
	...
	case vmName != "":
		client, err := b.provider.ComputeClient(ctx, b.Logger(), b.System(), subscriptionID)
		if err != nil {
			return err
		}
	...
	case resourceID != "":
		...
		client, err := b.provider.ResourceClient(ctx, b.Logger(), b.System(), subscriptionID)
		if err != nil {
			return err
		}
	...
	if _, ok := principalIDs[claims.ObjectID]; !ok {
		...
		c, err := b.provider.MSIClient(ctx, b.Logger(), b.System(), subscriptionID)
		if err != nil {
...
func (b *azureAuthBackend) getAPIVersionForResource(ctx context.Context, subscriptionID, resourceID string) (string, error) {
	...
	client, err := b.provider.ProvidersClient(ctx, b.Logger(), b.System(), subscriptionID)
	if err != nil {
		return "", err
	}
